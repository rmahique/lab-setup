...
echo "SUSE Manager Server Client bootstrap script v5.0.4"
...
#         wget -qO- https://mlm-server.geeko.land/pub/bootstrap/bootstrap-<edited_name>.sh | /bin/bash
#         ...or...
#         curl -Sks https://mlm-server.geeko.land/pub/bootstrap/bootstrap-<edited_name>.sh | /bin/bash
...
CLIENT_OVERRIDES=${CLIENT_OVERRIDES:-client-config-overrides.txt}
HOSTNAME=${HOSTNAME:-mlm-server.geeko.land}

ORG_CA_CERT=${ORG_CA_CERT:-RHN-ORG-TRUSTED-SSL-CERT}

USING_SSL=${USING_SSL:-1}
USING_GPG=${USING_GPG:-1}

REGISTER_THIS_BOX=${REGISTER_THIS_BOX:-1}
...
PROFILENAME="${PROFILENAME}"   # Empty by default to let it be set automatically.
...
if [ "$INSTALLER" == yum ]; then
    ...
    if [ "$Y_CLIENT_CODE_BASE" == almalinux ] ||       [ "$Y_CLIENT_CODE_BASE" == rockylinux ] ||       [ "$Y_CLIENT_CODE_BASE" == oracle ] ||       [ "$Y_CLIENT_CODE_BASE" == alibaba ] ||       [ "$Y_CLIENT_CODE_BASE" == openEuler ] ||       [ "$Y_CLIENT_CODE_BASE" == centos ] ; then
    ...
    if [ $VENV_ENABLED -eq 1 ]; then
        if [ -z "$SNAPSHOT_ID" ]; then
            ...
            call_tukit "zypper --non-interactive update venv-salt-minion zypper openssl ||:"
        fi
    else
        if [ -z "$SNAPSHOT_ID" ]; then
            zypper --non-interactive up salt salt-minion zypper openssl $RHNLIB_PKG ||:
        else
            call_tukit "zypper --non-interactive update salt salt-minion zypper openssl $RHNLIB_PKG ||:"
        fi
    fi
